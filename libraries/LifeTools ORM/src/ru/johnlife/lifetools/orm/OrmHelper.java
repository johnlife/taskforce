...
public class OrmHelper<ID> extends OrmLiteSqliteOpenHelper {
	...
	public <T extends AbstractData> void persistAll(List<T> list) {
		if (list == null || list.isEmpty()) return;
		Dao<T, ID> dao = getDataDao(list.get(0));
		for (T t : list) {
			try {
	...
	public <T extends AbstractData> void deleteAll(Class<T> dataClass) {
		try {
			getDataDao(dataClass).delete(getAll(dataClass));
		} catch (SQLException e) {
			handleException(e);
		}
	}
